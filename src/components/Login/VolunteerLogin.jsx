...
const VolunteerLogin = () => {
  ...
  const handleRequestOTP = async (e) => {
    e.preventDefault();
    if (!name.trim() || !volunteerId.trim() || !email.trim()) return;
    ...
    try {
      ...
      const generatedOtp = await requestOTP('volunteer', name.trim(), volunteerId.trim(), trimmedEmail);
      setStep(2);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      setAuthState({ role: 'volunteer', isAuthenticated: true, user });
    ...
    <div className="min-h-screen flex flex-col p-6 bg-slate-900 animate-fade-in relative items-center justify-center">
      ...
      <div className="w-full max-w-md bg-slate-800/50 p-8 rounded-3xl border border-slate-700 shadow-2xl backdrop-blur-sm">
        <div className="flex flex-col items-center mb-8">
          <div className="p-4 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 shadow-lg shadow-purple-500/20 mb-4">
            <UserCheck className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-3xl font-bold text-white">{t('volunteerLogin', 'Volunteer Login')}</h2>
        ...
        {step === 1 ? (
          <form onSubmit={handleRequestOTP} className="flex flex-col gap-5">
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('volunteerName', 'Volunteer Name')}</label>
              <input 
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors"
                placeholder={t('namePlaceholder', 'e.g. John Doe')}
                required
              />
            </div>
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('volunteerId', 'Volunteer ID')}</label>
              <input 
                type="text"
                value={volunteerId}
                onChange={(e) => setVolunteerId(e.target.value)}
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors"
                placeholder={t('volunteerIdPlaceholder', 'e.g. VOL-123')}
                required
              />
            </div>
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('registeredEmail', 'Registered Email')}</label>
              <input 
                type="email"
                value={email}
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full mt-4 py-4 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold text-lg hover:opacity-90 hover:scale-[1.02] transition-all duration-300 shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:scale-100"
            >
              {loading ? t('sending', 'Sending...') : t('sendOTP', 'Send OTP')}
            </button>
            <div className="text-center text-xs text-slate-500 mt-2">
              {t('testCredentialsVolunteer', 'Use VOL-123 / John Doe / john@example.com for testing')}
            </div>
          </form>
        ) : (
          <form onSubmit={handleVerifyOTP} className="flex flex-col gap-5">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors text-center text-2xl tracking-widest"
                placeholder="••••••"
                maxLength={6}
                required
              />
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                className="text-purple-400 text-sm font-medium hover:text-purple-300 transition-colors disabled:opacity-50"
              >
