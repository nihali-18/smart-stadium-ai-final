...
const StaffLogin = () => {
  ...
  const handleRequestOTP = async (e) => {
    e.preventDefault();
    if (!name.trim() || !staffId.trim() || !email.trim()) return;
    ...
    try {
      ...
      const generatedOtp = await requestOTP('staff', name.trim(), staffId.trim(), trimmedEmail);
      setStep(2);
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      setAuthState({ role: 'staff', isAuthenticated: true, user });
    ...
    <div className="min-h-screen flex flex-col p-6 bg-slate-900 animate-fade-in relative items-center justify-center">
      ...
      <div className="w-full max-w-md bg-slate-800/50 p-8 rounded-3xl border border-slate-700 shadow-2xl backdrop-blur-sm">
        <div className="flex flex-col items-center mb-8">
          <div className="p-4 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500 shadow-lg shadow-indigo-500/20 mb-4">
            <UserCheck className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-3xl font-bold text-white">{t('staffLogin', 'Staff Login')}</h2>
        ...
        {step === 1 ? (
          <form onSubmit={handleRequestOTP} className="flex flex-col gap-5">
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('staffName', 'Staff Name')}</label>
              <input 
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
                placeholder={t('namePlaceholder', 'e.g. John Doe')}
                required
              />
            </div>
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('staffId', 'Staff ID')}</label>
              <input 
                type="text"
                value={staffId}
                onChange={(e) => setStaffId(e.target.value)}
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
                placeholder={t('staffIdPlaceholder', 'e.g. STF-123')}
                required
              />
            </div>
            <div>
              <label className="block text-slate-300 font-medium mb-2">{t('officialStaffEmail', 'Official Staff Email')}</label>
              <input 
                type="email"
                value={email}
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full mt-4 py-4 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold text-lg hover:opacity-90 hover:scale-[1.02] transition-all duration-300 shadow-lg shadow-indigo-500/25 disabled:opacity-50 disabled:scale-100"
            >
              {loading ? t('sending', 'Sending...') : t('sendOTP', 'Send OTP')}
            </button>
            <div className="text-center text-xs text-slate-500 mt-2">
              {t('testCredentialsStaff', 'Use STF-123 / John Doe / john@example.com for testing')}
            </div>
          </form>
        ) : (
          <form onSubmit={handleVerifyOTP} className="flex flex-col gap-5">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-xl text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors text-center text-2xl tracking-widest"
                placeholder="••••••"
                maxLength={6}
                required
              />
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                className="text-indigo-400 text-sm font-medium hover:text-indigo-300 transition-colors disabled:opacity-50"
              >
