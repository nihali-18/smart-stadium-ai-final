...
import { ClipboardList, QrCode, AlertOctagon, Users, Search, User, LogOut } from 'lucide-react';
...
const VolunteerDashboard = () => {
  ...
  const widgets = [
    { id: 'tasks', title: t('assignedTasks', 'Assigned Tasks'), icon: ClipboardList, color: 'from-blue-500 to-indigo-500' },
    { id: 'incident', title: t('incidentReporting', 'Incident Reporting'), icon: AlertOctagon, color: 'from-orange-500 to-red-500' },
    { id: 'crowd', title: t('crowdManagement', 'Crowd Management'), icon: Users, color: 'from-cyan-500 to-blue-500' }
  ];
    ...
    <div className="min-h-screen flex flex-col p-6 bg-slate-900 animate-fade-in relative">
      <div className="flex items-center justify-between mb-8 gap-4 pt-4 pl-4 pr-24 md:pl-8 md:pr-48">
        <h2 className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 text-3xl md:text-4xl">
          {t('volunteerDashboard', 'Volunteer Dashboard')}
        </h2>
        <button 
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto w-full px-4 md:px-8">
        {widgets.map((widget, index) => {
          ...
          return (
            <button
              key={widget.id}
              onClick={() => navigate(`/dashboard/volunteer/${widget.id}`)}
